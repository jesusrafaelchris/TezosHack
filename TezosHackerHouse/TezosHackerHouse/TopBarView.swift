...
class TopBarView: UIView {
    ...
    lazy var dateLabel: UILabel = {
        let text = UILabel()
        text.layout(colour: .darkGray, size: 14, text: "Welcome back", bold: false)
        return text
    }()

    lazy var titleLabel: UILabel = {
        let text = UILabel()
        text.layout(colour: .black, size: 16, text: "Hi, Artemiy!", bold: true)
        return text
    }()
    
    lazy var notificationButton: UIButton = {
        let button = UIButton()
        button.setsizedImage(symbol: "bell.badge", size: 14, colour: .black)
        return button
    }()
    ...
    override init(frame: CGRect) {
        ...
        profileImage.anchor(top: nil, paddingTop: 0, bottom: nil, paddingBottom: 0, left: leftAnchor, paddingLeft: 36, right: nil, paddingRight: 0, width: 0, height: 0)
